    ...
    F: Fn(&T, &T) -> Ordering,
{
    if x.len() > 1 {
        let mid = x.len() / 2;
        do_sort(&mut x[..mid], true, comparator);
        do_sort(&mut x[mid..], false, comparator);
    ...
    F: Fn(&T, &T) -> Ordering,
{
    if x.len() > 1 {
        ...
        let mid = x.len() / 2;
        sub_sort(&mut x[..mid], forward, comparator);
        sub_sort(&mut x[mid..], forward, comparator);
    }
}
    ...
    F: Fn(&T, &T) -> Ordering,
{
    ...
    let mid = x.len() / 2;
    for i in 0..mid {
        if comparator(&x[i], &x[mid + i]) == swap_condition {
            x.swap(i, mid + i);
        }
    }
}
...
mod tests {
    use super::*;
    use crate::ch03::util::{is_sorted, new_u32_vec};
    use crate::ch03::SortOrder::*;
